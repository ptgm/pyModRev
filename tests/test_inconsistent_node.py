...
def test_initial_values(nodes):
    n1, n2 = nodes
    assert n1.get_id() == 'node1'
    assert n1.get_generalization()
    assert not n2.get_generalization()
    assert not n1.is_repaired()

def test_set_repair_type(nodes):
    n1, _ = nodes
    n1.set_repair_type(3)
    assert n1.get_repair_type() == 3
...
def test_add_repair_set(nodes):
    ...
    assert n1.get_n_repair_operations() == 5
    
    # Better repair set
    rs2 = RepairSet()
    rs2.n_repair_operations = 3
    n1.add_repair_set(rs2)
    assert n1.get_n_repair_operations() == 3
    assert len(n1.get_repair_set()) == 1

def test_topological_error(nodes):
    n1, _ = nodes
    n1.set_topological_error(True)
